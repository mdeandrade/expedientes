...
$values = array_merge($values,$_FILES);
	...
	function executeAdd($values = null,$errors = array())
	{
		...
		executeNew($values,$errors);die;
                ...
                //print_r($values);die;
                $Grupos = new Grupos();
                $values = $Grupos->saveGrupos($values);
	...
	function executeEdit($values = null,$errors = null,$msg = null)
	{
		//print_r($values);die;
		$Grupos = new Grupos();
		$values = $Grupos->getGruposById($values);
	...
	{
		$Grupos= new Grupos();
		$list_json = $Grupos ->getGruposList($values);
		$list_json_cuenta = $Grupos ->getCountGruposList($values);
		$array_json = array();
		...
		if(count($list_json)>0)
		{
			foreach ($list_json as $list) 
			{
				...
				$id_grupo = $list['id_grupo'];
				$array_json['data'][] = array(
					"id_grupo" => $id_grupo,
					"nom_grupo" => $list['nom_grupo'],
					"nom_estatus" => $list['nom_estatus'],
					"actions" => 
                                       '<form method="POST" action = "'.full_url.'/ap/Grupos/index.php" >'
