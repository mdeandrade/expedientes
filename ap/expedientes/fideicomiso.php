...
<?php foreach($data_archivo as $data):?>
  ...
  <div class="form-group col-sm-4" id="div_<?php echo $data['nom_variable']."_".$data['id_tipdoc'];?>">
        <label><?php echo $data['nom_tipdoc']?> 
            ...
            <a onclick="sumaCampo('<?php echo $data['nom_variable']?>','<?php echo $data['id_tipdoc']?>');">[ + ]</a></b>
            <?php endif; ?>
        </label>
        <?php if(isset($values['id_persona']) and $values['id_persona']!=''):?>
        
            <?php $listado_archivos = $ExpedientesDetalles->getDocumentosExpedienteArchivo($values['id_persona'],'fideicomiso.php',$data['id_folio'],$data['id_tipdoc']);?>
