<?php
	...
	class Ubicaciones {
		...
		public function getUbicacionesList($values)
		{	
			$columns = array();
			...
			{
				$where.=" AND id_ubicacion = ".$values['columns'][0]['search']['value']."";
				//echo $values['columns'][0]['search']['value'];die;
			}
			if(isset($values['columns'][1]['search']['value']) and $values['columns'][1]['search']['value']!='')
			{
				$where.=" AND upper(nom_ubicacion) like ('%".$values['columns'][1]['search']['value']."%')";
				//echo $values['columns'][0]['search']['value'];die;
			}			
			...
			->select("*,nom_ubicacion, nom_estatus")
			->order("$column_order $order")
                        ->join("estatus","INNER JOIN estatus e on e.id_estatus = ubicaciones.id_estatus")        
			->where("$where")
			->limit($limit,$offset);
			//echo $q;die;
			return $q; 			
		...
		public function getUbicacionesById($values){
			...
			$q = $ConnectionORM->getConnect()->ubicaciones
			->select("*")
			->where("ubicaciones.id_ubicacion=?",$values['id_ubicacion'])
			->fetch();
			return $q; 				
		...
		function saveUbicaciones($values){
                        ...
                        $array = array(
                            "nom_ubicacion" => $values['nom_ubicacion'],
                            "id_estatus" => $values['id_estatus']
                            
                        );
			$ConnectionORM = new ConnectionORM();
			$q = $ConnectionORM->getConnect()->ubicaciones()->insert($array);
			$values['id_ubicacion'] = $ConnectionORM->getConnect()->ubicaciones()->insert_id();
			return $values;	
		...
		function updateUbicaciones($values){
			...
			$id_ubicacion = $values['id_ubicacion'];
                        //echo $id_ubicacion;die;
			$ConnectionORM = new ConnectionORM();
			$q = $ConnectionORM->getConnect()->ubicaciones("id_ubicacion", $id_ubicacion)->update($array);
			return $q;
		...
		public function getUbicacionesOperatorList($values)
		{	
			$columns = array();
			$columns[0] = 'id_user';
			$columns[1] = 'login';
		...
		public function getCountUbicacionesOperatorList($values)
		{	
			$where = '1 = 1';
			if(isset($values['search']['value']) and $values['search']['value'] !='')
			{	
