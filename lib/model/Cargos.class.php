<?php
	...
	class Cargos {
		...
		public function getCargosList($values)
		{	
			$columns = array();
			$columns[0] = 'id_cargo';
			$columns[1] = 'nom_cargo';
			...
			{
				$where.=" AND id_cargo = ".$values['columns'][0]['search']['value']."";
				//echo $values['columns'][0]['search']['value'];die;
			}
			if(isset($values['columns'][1]['search']['value']) and $values['columns'][1]['search']['value']!='')
			{
				$where.=" AND upper(nom_cargo) like ('%".$values['columns'][1]['search']['value']."%')";
				//echo $values['columns'][0]['search']['value'];die;
			}			
			...
			->select("*,nom_cargo, nom_estatus")
			->order("$column_order $order")
                        ->join("estatus","INNER JOIN estatus e on e.id_estatus = cargos.id_estatus")        
			->where("$where")
			->limit($limit,$offset);
			//echo $q;die;
			return $q; 			
		}
		public function getCountCargosList($values)
		{	
			$where = '1 = 1';
			if(isset($values['columns'][0]['search']['value']) and $values['columns'][0]['search']['value']!='')
			{
		...
		public function getCargosById($values){
			$ConnectionORM = new ConnectionORM();
			$q = $ConnectionORM->getConnect()->cargos
			->select("*")
			->where("cargos.id_cargo=?",$values['id_cargo'])
			->fetch();
			return $q; 				
		...
		function saveCargos($values){
			 
                        $array = array(
                            "nom_cargo" => $values['nom_cargo'],
                            "id_estatus" => $values['id_estatus']
                            
                        );
			$ConnectionORM = new ConnectionORM();
			$q = $ConnectionORM->getConnect()->cargos()->insert($array);
			$values['id_cargo'] = $ConnectionORM->getConnect()->cargos()->insert_id();
			return $values;	
			
		}
		function updateCargos($values){
			...
			$id_cargo = $values['id_cargo'];
                        //echo $id_cargo;die;
			$ConnectionORM = new ConnectionORM();
			$q = $ConnectionORM->getConnect()->cargos("id_cargo", $id_cargo)->update($array);
			return $q;
		...
		public function getCargosOperatorList($values)
		{	
			$columns = array();
			$columns[0] = 'id_user';
			$columns[1] = 'login';
		...
		public function getCountCargosOperatorList($values)
		{	
			$where = '1 = 1';
			if(isset($values['search']['value']) and $values['search']['value'] !='')
			{	
