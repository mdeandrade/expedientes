<?php
	...
	class ExpedientesDetalles {
		...
		public function getExpedienteDetalleById($values){
		...
		function deleteExpedienteDetalle($id_expediente, $id_folio, $id_tipdoc, $ubi_docserver){
			unset($values['action']);
			$ConnectionORM = new ConnectionORM();
                        $query = "DELETE FROM expedientes_detalles where id_expediente= $id_expediente and id_folio=$id_folio and id_tipdoc = $id_tipdoc and ubi_docserver= '$ubi_docserver'";
			$q = $ConnectionORM->ejecutarPreparado($query);
			
			
		}		
		function saveExpedientesDetalles($id_expediente,$id_folio,$id_tipdoc,$ubi_docserver,$id_estatus){
			 
                        $array = array(
                            "id_expediente" => $id_expediente,
                            "id_folio" => $id_folio,
                            "id_tipdoc" => $id_tipdoc,
                            "ubi_docserver" => $ubi_docserver,
                            "fec_creado" => date('Y-m-d'),
                            "id_estatus" => $id_estatus
                            
                        );
			$ConnectionORM = new ConnectionORM();
			$q = $ConnectionORM->getConnect()->expedientes_detalles()->insert($array);
			//$values['id_expediente'] = $ConnectionORM->getConnect()->expedientes_detalles()->insert_id();
			return $values;	
			
		}
		public function getDocumentosExpedienteArchivo($id_persona,$nom_archivo,$id_folio,$id_tipdoc){
			$ConnectionORM = new ConnectionORM();
			$q = $ConnectionORM->getConnect()->expedientes_detalles
			->select("*")
                        ->join("expedientes","INNER JOIN expedientes on expedientes.id_expediente = expedientes_detalles.id_expediente")
                        ->join("folios","INNER JOIN folios on folios.id_folio = expedientes_detalles.id_folio ")
			->where("id_persona=?",$id_persona)
                        ->and("archivo=?",$nom_archivo)
                        ->and("expedientes_detalles.id_folio=?",$id_folio)
                        ->and('id_tipdoc=?',$id_tipdoc);
                       // echo $id_persona."-----".$nom_archivo."-id_folio=".$id_folio."---id_tipdoc=".$id_tipdoc;
                        //echo $q;die;
			return $q; 				
